(function () {
  ...
  function runtime() {
    ...
    const blank = () => ({ version: 1, fields: { objekt: '', datum: today(), sachbearbeiter: '' }, rows: [blankRow()] });
    function normalize(data) {
      const source = isObject(data) ? clone(data) : {};
      const fields = isObject(source.fields) ? source.fields : {};
      const seen = new Set();
      return { ...source, version: 1, fields: { ...fields, ...Object.fromEntries(['objekt', 'datum', 'sachbearbeiter'].map(k => [k, String(fields[k] ?? '')])) },
    ...
    function render() {
      ['objekt', 'datum', 'sachbearbeiter'].forEach(k => { document.getElementById(k).value = state.fields[k]; });
    ...
    function renderArchive() {
      ...
      list.innerHTML = entries.length ? entries.map(e => {
        ...
        return '<article class="archive-item archive-item-detailed' + (current ? ' archive-item-current' : '') + '" data-archive-id="' + esc(e.id) + '"' + (current ? ' aria-current="true"' : '') + '>' +
          ...
          '<div class="archive-detail-grid"><span class="archive-detail-pair"><b>Gebäude / Gruppe: </b><span>' + esc(e.meta?.anlage || 'nicht vorhanden') + '</span></span></div>' +
    ...
    async function exportPdf() {
      ...
      try {
        ...
        for (position = 0; position < data.rows.length; position++) {
          ...
          textRow('Bauvorhaben / Objekt', data.fields.objekt);
          textRow('Datum / Bearbeiter', [val(data.fields.datum), val(data.fields.sachbearbeiter)].join(' / '));
